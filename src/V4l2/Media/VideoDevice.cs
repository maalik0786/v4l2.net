...
using Iot.Device.Media.Media.Devices;
...
public abstract partial class VideoDevice : IDisposable
{
	...
	public static VideoDevice Create(VideoConnectionSettings settings) =>
		new UnixVideoDevice(settings);
	...
	public abstract IEnumerable<(uint Width, uint Height)> GetPixelFormatResolutions(
		PixelFormat format);
